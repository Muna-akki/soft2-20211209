...
int main(int argc, char **argv){
    ...
    if(argc>3){
  	...
  	size_t objnum = strtol(argv[1],NULL,10);
...
void read_file(FILE* fp, Object objs[], const size_t numobj, const Condition cond){
    ...
    for(int i=0 ; i<numobj ; i++){
        double m,x,y,vx,vy;
        int n = fscanf(fp, "%lf %lf %lf %lf %lf",&m, &x, &y, &vx, &vy);
        fscanf(fp,"%*[^\n]%*c");
        if(n!=5 || check == 1){
            check = 1;
            objs[i] = (Object){.m = 0.0, .x = cond.width, .y = cond.height, .vx = 0, .vy = 0};
        }else{
            objs[i] = (Object){ .m = m, .x = x, .y = y, .vx = vx, .vy = vy};
        }
    }

}

void collision(Object objs[], const size_t numobj, const Condition cond){
    //iとjが一定距離min以下でiにjが融合
    double dr = 0;
    double min = 1;
    for(int i=0 ; i<numobj ; i++){
        if(objs[i].m==0){
        ...
        for(int j=i+1 ; j<numobj ; j++){
            dr = sqrt((objs[i].x-objs[j].x)*(objs[i].x-objs[j].x)+(objs[i].y-objs[j].y)*(objs[i].y-objs[j].y));
            if(dr>min){
                continue;
            }
            ...
            //融合された側は質量が二つの物体の和で運動量保存則を満たすような動きをする。
            objs[i].m = M;
            objs[i].prev_y = objs[i].y;
            objs[i].prev_x = objs[i].x;
            objs[i].vy = vy;
            objs[i].vx = vx;
            objs[i].prev_vx = prev_vx;
            objs[i].prev_vy = prev_vy;
            
            //融合した側は質量0物体として画面外に出てもらう。
            objs[j].m = 0;
            objs[j].x = (double)cond.width;
            objs[j].y = (double)cond.height;
            objs[j].prev_y = (double)cond.height;
            objs[j].prev_x = (double)cond.width;
            objs[j].vx = 0;
            objs[j].vy = 0;
            objs[j].prev_vx = 0;
            objs[j].prev_vy = 0;
